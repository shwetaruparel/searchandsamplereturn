...
def update_rover(Rover, data):
      ...
      #Rover.sample_located += Rover.samples_collected
     ...
     # 'samples collected:', Rover.samples_collected,'samples_located',Rover.samples_located)
...
def create_output_images(Rover):
      ...
      if rock_world_pos[0].any():
        ...
        rock_world_pos = np.unique(rock_world_pos,axis = 0)
        rock_size = 2  
        
        for idx in range(len(Rover.samples_pos[0])):
            test_rock_x = Rover.samples_pos[0][idx]
            test_rock_y = Rover.samples_pos[1][idx]
           # print("Rocks detection......."+str(np.unique(rock_world_pos))+"\n")
            #print("Check this where function" + str(np.isin(np.unique(rock_world_pos[0]),Rover.samples_pos[0])))
            for rdx in range(len((rock_world_pos[0]))):
                print("Rock distance to check =" +str(rock_world_pos)+"\n")
                rock_sample_dists = np.sqrt((test_rock_x - rock_world_pos[1][rdx])**2 + \
                        (test_rock_y - rock_world_pos[0][rdx])**2)
                #print("Rock sample distance" +str(rock_sample_dists)+"\n")
          # If rocks were detected within 3 meters of known sample positions
          # consider it a success and plot the location of the known
          # sample on the map
          #
                print ("The sample located is :" +str(Rover.samples_pos[0][idx])+"," + str(Rover.samples_pos[1][idx])+"\n")
                print ("Rock Sample Distance : " +str(rock_sample_dists))
                if rock_sample_dists < 3:
                    print ("The sample position is :" +str(Rover.samples_pos[0][idx])+"," +str(Rover.samples_pos[1][idx]))
                    samples_located += 1
                    map_add[test_rock_y-rock_size:test_rock_y+rock_size, 
                            test_rock_x-rock_size:test_rock_x+rock_size, :] = 255
                #Rover.near_sample = True
            
      if(samples_located > Rover.samples_collected):
          Rover.samples_located  = samples_located
      else:
          Rover.samples_located = Rover.samples_collected
      ...
      print (" Toatl navigable pix" + str(tot_nav_pix),"Good Nav Pix" + str(good_nav_pix)+"Total map pix" + str(tot_map_pix))
      if tot_nav_pix > 0:
