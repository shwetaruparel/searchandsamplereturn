...
def perspect_transform(img, src, dst):
    ...
    masked = cv2.warpPerspective(np.ones_like(img[:,:,0]), M, (img.shape[1], img.shape[0]))# keep same size as input image
...
#    obs_map = np.absolute(np.float32(threshed)-1)*masked  
    obs_map =np.invert(np.invert(threshed)+masked)
...
# Extract navigable terrain pixels
    ...
    worldmap = Rover.worldmap.shape[0]
    scale = 2*dst_size
...
# Add pixel positions to worldmap
   ...
   # mean_dir = np.mean(angles)
    ...
    if rock_map.any():
        ...
        rock_y_world, rock_x_world = pix_to_world(rock_xpix, rock_ypix, rover_xpos, 
                                rover_ypos, rover_yaw, 
                                worldmap, scale)
       ...
       # print ('Rocks Map' , Rover.worldmap[rock_ycen,rock_xcen,1])
       
       # print('rock x pos=',rock_xpix, 'rock y pix',rock_ypix,'rock distance' , rock_dist)
        Rover.rock_traced = True
    else:
        Rover.vision_image[:,:,1] = 0
        Rover.rock_traced = False

        #Rover.worldmap[y_world,x_world] = 1 
   # print('Near_sample=', Rover.near_sample,'Xpos =', rover_xpos, 'Ypos =', rover_ypos, 'Yaw =', rover_yaw)
    return Rover
